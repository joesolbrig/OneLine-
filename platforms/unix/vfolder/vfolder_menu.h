...
public:
    ...
    static QString xdgDirPath(QString dirStr){
        QString p =
                xdgBaseDir.absoluteFilePath() + "/" + dirStr + "/" + xdgPrefix;
        return p;

    }
    static QString allXdgDirs(QString dirStr, QString extension){
        QString p =
                xdgBaseDir.absoluteFilePath() + "/" + dirStr + "/" + xdgPrefix;
        QDir d(p);
    ...
    static QString realPath(QString p){
        QFileInfo f(p);
        return f.absoluteFilePath();
    }
    static QString findDirs(){
...
public:
  ...
  public:
     ...
     QHash<QString,CatItem> excludeItems; // Needed when merging due to Move.
...
public:
  QStringList m_rawEnvironment; //Now derive everything from this using the standard...
  ...
  public:
     ...
     QHash<QString,QList<CatItem> > dictCategories; // category -> apps
     QHash<QString,CatItem> applications; // rel path -> service
     QHash<CatItem,QString> appRelPaths; // service -> rel path
  };
...
private:
   ...
   */
  CatItem findApplication(const QString &relPath);

  /**
   * Lookup applications by category
   */
  QList<CatItem*> findCategory(const QString &category);

  /**
   * Add new application
   */
  void addApplication(const QString &id, CatItem service);
   ...
   */
  void includeItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2);

  /**
   * Remove all items from the items1 set that aren't also in the items2 set
   */
  void matchItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2);

  /**
   * Remove all items in the items2 set from the items1 set
   */
  void excludeItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2);
   ...
   */
  void insertService(SubMenu *parentMenu, const QString &name, CatItem newService);
   ...
   */
  void markUsedApplications(const QHash<QString,CatItem>& items);
  ...
  void processCondition(QDomElement &docElem, QHash<QString,CatItem>& items);
