/*  This file is derived from parts of the KDE libraries
 *  original Copyright (C) 2003 Waldo Bastian <bastian@kde.org>
 *
 * Modified by Hans Joseph Solbrig (c) 2010 <joesolbrig@yahoo.com>
 * Xdg is an excessively complex standard. Only Kde has an interface
 * the access it and that's burried in kde-daemon, something it would
 * be nice not to have to load to use.
 *
...
static void
track(const QString &menuId, const QString &menuName, const QHash<QString,CatItem>& includeList, const QHash<QString,CatItem>& excludeList, const QHash<QString,CatItem>& itemList, const QString &comment)
{
...
void
VFolderMenu::includeItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2)
{
   foreach (const CatItem &p, items2) {
       items1.insert(p.fullPath, p);
   }
}

void
VFolderMenu::matchItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2)
{
   foreach (const CatItem &p, items1)
   {
       QString id = p.fullPath;
...
void
VFolderMenu::excludeItems(QHash<QString,CatItem>& items1, const QHash<QString,CatItem>& items2)
{
   foreach (const CatItem &p, items2)
       items1.remove(p.fullPath);
}
...
void
VFolderMenu::insertService(SubMenu *parentMenu, const QString &name, CatItem newService)
{
   ...
   {
     // Add it here
     parentMenu->items.insert(newService.fullPath, newService);
     return;
   }
    ...
    m_track(false)
{
   ...
   m_rootMenu = 0;
   m_rawEnvironment = QProcessEnvironment::systemEnvironment().toStringList();
...
#define FOR_ALL_APPLICATIONS(it) \
   ...
   { \
      QHashIterator<QString,CatItem> it = info->applications; \
      while (it.hasNext()) \
      { \
         it.next();
...
VFolderMenu::findApplication(const QString &relPath)
{
   foreach(AppsInfo *info, m_appsInfoStack)
   {
      if (info->applications.contains(relPath)) {
         CatItem s = info->applications[relPath];
         if (!s.isEmpty())
            return s;
      }
   }
...
void
VFolderMenu::addApplication(const QString &id, CatItem service)
{
   service.fullPath = (id);
...
VFolderMenu::buildApplicationIndex(bool unusedOnly)
{
   foreach (AppsInfo *info, m_appsInfoList)
   {
      ...
      QMutableHashIterator<QString,CatItem> it = info->applications;
      while (it.hasNext())
      {
         CatItem s = it.next().value();
         if (unusedOnly && m_usedAppsDict.contains(s.fullPath))
         {
         ...
         QList<CatItem> cats = s.getCategories();

         //const QStringList cats = s->categories();
         for(QList<CatItem>::ConstIterator it2 = cats.begin();
             it2 != cats.end(); ++it2)
         {
            QString cat = (*it2).shortName;
...
VFolderMenu::absoluteDir(const QString &_dir, const QString &baseDir, bool keepRelativeToCfg)
{
    ...
    //dir = KGlobal::dirs()->findResource("xdgconf-menu", dir);
        dir = LGlobal::xdgDirPath(dir);


    }

   dir = LGlobal::absoluteFilePath(dir);
...
{
qDebug() << "VFolderMenu::mergeFile:" << m_docInfo.path;
...
//         const QStringList dirs = KGlobal::dirs()->findDirs("xdgconf-menu", dir);
         const QStringList dirs = LGlobal::xdgDirPath(dir);
...
void
VFolderMenu::processCondition(QDomElement &domElem, QHash<QString,CatItem>& items)
{
   if (domElem.tagName() == "And")
   {
      ...
      QHash<QString,CatItem> andItems;
      while( !n.isNull() ) {
         QDomElement e = n.toElement();
         if (e.tagName() == "Not")
         {
   ...
   else if (domElem.tagName() == "Not")
   {
      ...
      {
         CatItem s = it.value();
         items.insert(s.fullPath, s);
      }
      FOR_ALL_APPLICATIONS_END

      QHash<QString,CatItem> notItems;
      QDomNode n = domElem.firstChild();
      while( !n.isNull() ) {
         QDomElement e = n.toElement();
         if ( !e.isNull() ) {
   ...
   else if (domElem.tagName() == "Filename")
   {
      ...
      CatItem s = findApplication(filename);
      if (s.isEmpty())
       ...
       it != relFiles.constEnd(); ++it)
   {
      ...
      if ((*it).endsWith(QLatin1String(".desktop")))
      {
         ...
         if (service && !m_forcedLegacyLoad)
         {
            ...
            items.insert(service.fullPath, service);
   ...
   if (pass == 0)
   {
      ...
      while( !n.isNull() ) {
         ...
         else if (e.tagName() == "KDELegacyDirs")
         {
            ...
            {
qDebug() << "Processing KDE Legacy dirs for <KDE>";
         ...
         else if (e.tagName() == "LegacyDir")
         {
            ...
            if (m_defaultLegacyDirs.contains(dir))
            {
               if (!kdeLegacyDirsDone)
               {
qDebug() << "Processing KDE Legacy dirs for" << dir;
                  SubMenu *oldMenu = m_currentMenu;
                  m_currentMenu = new SubMenu;
...
static QString parseAttribute( const QDomElement &e)
{
    ...
    if ( e.hasAttribute( "inline" ) )
    {
        ...
        if (  str=="true" )
            ...
            qDebug()<<" Error in parsing inlibe attribute :"<<str;
    }
...
void
VFolderMenu::markUsedApplications(const QHash<QString,CatItem>& items)
{
   foreach(const CatItem &p, items)
      m_usedAppsDict.insert(p.fullPath);
}
