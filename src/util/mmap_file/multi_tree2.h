...
template <class VALUE, class INDEX_TYPE> class Bucket {
        QHash< Hash_id, QList<VALUE> > itemBucket;
        QHash< Hash_id, short> itemBucketLength;
        qint32 size;
    public:
        Bucket(){
            size = 0;
        ...
        int add(VALUE v){
            ...
            itemBucket[i].append(v);
            itemBucketLength[i]= itemBucketLength[i]+1;
            size++;
            return ((itemBucket[i].size()) -1);
        }

        bool is_at(VALUE v, Hash_id id){
            if(!itemBucket.contains(id))
                { return false; }

            for(int i=0; i < itemBucket[id].size(); i++){
                if(itemBucket[id][i] == v)
                    { return true; }
            }
            return false;
        }
        ...
        Sub_id subkey(INDEX_TYPE t){
            Hash_id id = hHash(t);
            if(!itemBucket.contains(id)) { return -1; }
        ...
        Sub_id subkey(VALUE v){
            INDEX_TYPE t = v.getId();
            Hash_id id = hHash(t);
            if(!itemBucket.contains(id)) { return -1; }

            for(int i=0; i < itemBucket[id].size(); i++){
                if(itemBucket[id][i] == t) { return i; }
            }
            return -1;
        }
        ...
        int len_at_loc(Hash_id i){
            return itemBucket[i].size();
        }
