...
public:
    FlexTextEdit(QWidget* p): QTextEdit(p){
...
public:
    MyFullTextEdit(QWidget* parent) : QWidget(parent,Qt::Widget), usingShortEdit(true){
        ...
        longEdit = new FlexTextEdit(parent);
        ...
        shortEdit->setAlignment(Qt::AlignTop);
        this->raise();
    ...
    QMargins contentsMargin(){
        if(usingShortEdit){
            return shortEdit->contentsMargins();
        } else {
            return longEdit->contentsMargins();
        }
    }

    void setContentsMargin(QMargins m){
        if(usingShortEdit){
            shortEdit->setContentsMargins(m);
        } else {
            longEdit->setContentsMargins(m);
        }
    }
