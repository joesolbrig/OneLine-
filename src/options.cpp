    ...
    : QDialog(parent) 
{
    setupUi(this);
    curPlugin = -1;
    MainUserWindow* main = gMainWidget;
    if (main == NULL) return;
    ...
    QString skinName = gSettings->value("GenOps/skin", gDirs->value("defSkin")[0]).toString();
    ...
    foreach(QString szDir, gDirs->value("skins")) {
	QDir dir(szDir);
    ...
    if (CatBuilder::totalItems() > 0) {
	QString txt = tr("Index has ");
    txt += QString::number(CatBuilder::totalItems());
	txt += tr(" items");
	catSize->setText(txt);
    }
//    if (gBuilder != NULL) {
...
//    }
    ...
    Catalog::plugins.loadPlugins(*gDirs);
    foreach(PluginInfo info, Catalog::plugins.getPlugins()) {
    ...
    aboutVer->setText(QString(tr("This is version ")) + QString(ONELINE_VERSION));
...
OptionsDlg::~OptionsDlg() {
    ...
    //		Catalog::plugins.endDialog(item->data(3).toUInt());
    //	}
}
...
void OptionsDlg::accept() {
    ...
    bool newKey = main->setHotkey(iMetaKeys[genModifierBox->currentIndex()],
                                  iActionKeys[genKeyBox->currentIndex()]);
    if (!newKey) {
            QMessageBox::warning(this, tr(APPLICATION_NAME),
                tr("The hotkey you have chosen is already in use, please select another."));
            return;
    }
    ...
    // Apply Skin Options
    QString prevSkinName = gSettings->value("GenOps/skin", gDirs->value("defSkin")[0]).toString();
    ...
    //QHash<QString, PluginInfo> plugins = Catalog::plugins.getPlugins();
    bool changed = false;
    ...
    if (curPlugin >= 0) {
	QListWidgetItem* item = plugList->item(curPlugin);
    Catalog::plugins.endDialog(item->data(Qt::UserRole).toUInt(), true);
    }
...
void OptionsDlg::pluginChanged(int row) {
    ...
    QWidget* win = Catalog::plugins.doDialog(plugBox, item->data(Qt::UserRole).toUInt());
...
void OptionsDlg::pluginItemChanged(QListWidgetItem* iz) {
    ...
    Catalog::plugins.doDialog(plugBox, iz->data(Qt::UserRole).toUInt());
    }
...
void OptionsDlg::catalogBuilt() {
    if (CatBuilder::totalItems() > 0)
    catSize->setText(tr("Index has ") + QString::number(CatBuilder::totalItems()) + tr(" items"));
}
...
void OptionsDlg::catRescanClicked(bool val) {
    ...
    gSettings->endArray();
    //Full datastore rescan
    gMainWidget->refreshExtendCatalog(true);
}
...
void OptionsDlg::reject() {
    ...
    if (curPlugin >= 0) {
	QListWidgetItem* item = plugList->item(curPlugin);
    Catalog::plugins.endDialog(item->data(Qt::UserRole).toUInt(), false);
    }
